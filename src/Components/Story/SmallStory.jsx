...
const SmallStory = ({storyImg, name, time, avatar}) => {
  return (
    <div
    ...
    >
      <div className="w-full h-full flex items-end z-10">
        <div className="bg-(--color-gray) w-full rounded-[60px] h-[20%] p-[10px] flex items-center justify-between">
          <div className="flex gap-1 h-full items-center">
            <img src={avatar} className="h-full rounded-full" alt="" />
            <div className="flex flex-col gap-0">
              <p className="leading-none font-bold text-(--color-violet)">
                {name}
              </p>
              <p className="leading-none flex items-center gap-0.5 text-(--color-violet)">
                {time} <IoTimeOutline />
              </p>
            </div>
          </div>
